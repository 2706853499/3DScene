...
void ObjLoader::load(const char * path)
{
	...
	//第几个网格体
	int meshIndex = -1;
	Mesh mesh;            //facescape obj数据 没有此项
	mesh.name = "gmesh";
	//加入到mesh数组中
	meshIndex++;
	meshArr.push_back(mesh);
	currentMtl = "material_0";
	//读obj文件
	while (getline(objFile, line))
	{
		...
		{
			Vector3 vextex;
			vextex.x = (GLfloat)atof(arr[1].c_str())/100+2;
			vextex.y = (GLfloat)atof(arr[2].c_str())/100+2;
			vextex.z = (GLfloat)atof(arr[3].c_str())/100+2;
		...
		if (arr[0] == ("f")) {
			...
			for (int i = 1; i < arr.size(); i++) {
				...
				//int indexVN = atoi(indexs[2].c_str());  //去掉法相
	...
	std::cout <<"mtlFileName"<< mtlFilePath << endl;
...
void ObjLoader::render()
{
	...
	glEnd();
	Mat img = imread("1_neutral.bmp", CV_LOAD_IMAGE_COLOR);

	 
	//Mat dst_image = HSV2RGB(img);
			...
			if (type == TRANGLE) {
				...
				glBegin(GL_TRIANGLES);
				{
					...
					//	glTexCoord2f(textureArr[vtIndex].x, textureArr[vtIndex].y);
						...
						int t1 = int(textureArr[vtIndex].x * img.cols);

						int t2 = img.rows- int(textureArr[vtIndex].y * img.rows);
						 
						
						//uchar* temp_ptr = &((uchar*)(img->imageData + img->widthStep*((int)t2)))[(int)t1*3];
						//glColor3f((float)temp_ptr[2] / 256, (float)temp_ptr[1] / 256, (float)temp_ptr[0] / 256);
						glColor3f((GLfloat)img.at<Vec3b>(t2,t1)[2] / 256, (GLfloat)img.at<Vec3b>(t2, t1)[1] / 256, (GLfloat)img.at<Vec3b>(t2, t1)[0] / 256);
	...
	glBindTexture(GL_TEXTURE_2D, 0);  //如使用glColor3f，则需绑定纹理，如使用glTexCoord2f，则关闭//使用glTexCoord2f，效果更好
}

//初始化材质
void ObjLoader::initTexture()
{
	string mtlName = "1_neutral.obj.mtl";//当前初始化的材质，以免重复
	...
	for (int i = 0; i < meshArr.size(); i++) {
		for (int j = 0; j < meshArr[i].faceArr.size(); j++) {
			...
			if (meshArr[i].faceArr[j].texture.name != mtlName) {
				...
				//初始化
				//加载图片
				textureID = SOIL_load_OGL_texture("1_neutral.bmp",
					SOIL_LOAD_AUTO,
					SOIL_CREATE_NEW_ID,
